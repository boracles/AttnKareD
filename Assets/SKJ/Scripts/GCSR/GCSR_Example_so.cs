...
{
	public class GCSR_Example_so : MonoBehaviour
	{
		public PlayMakerFSM goFsm;		

		private GCSpeechRecognition _speechRecognition;		
		
		private string[] allDeviceName;
		...
		private void Start()
		{		
			string currentDevice = SystemInfo.deviceName;
			setApi(currentDevice);
			...
			_startRecordButton = GameObject.Find("Canvas/Button_StartRecord").GetComponent<Button>();
            _stopRecordButton = GameObject.Find("Canvas/Button_StopRecord").GetComponent<Button>();
            _getOperationButton = GameObject.Find("Canvas/Button_GetOperation").GetComponent<Button>();
            _getListOperationsButton = GameObject.Find("Canvas/Button_GetListOperations").GetComponent<Button>();
            _detectThresholdButton = GameObject.Find("Canvas/Button_DetectThreshold").GetComponent<Button>();
            _cancelAllRequestsButton = GameObject.Find("Canvas/Button_CancelAllRequests").GetComponent<Button>();
            _recognizeButton = GameObject.Find("Canvas/Button_Recognize").GetComponent<Button>();
            _refreshMicrophonesButton = GameObject.Find("Canvas/Button_RefreshMics").GetComponent<Button>();

            _speechRecognitionState = GameObject.Find("Canvas/Image_RecordState").GetComponent<Image>();

            _resultText = GameObject.Find("Canvas/Panel_ContentResult/Text_Result").GetComponent<Text>();

            _voiceDetectionToggle = GameObject.Find("Canvas/Toggle_DetectVoice").GetComponent<Toggle>();
            _recognizeDirectlyToggle = GameObject.Find("Canvas/Toggle_RecognizeDirectly").GetComponent<Toggle>();
            _longRunningRecognizeToggle = GameObject.Find("Canvas/Toggle_LongRunningRecognize").GetComponent<Toggle>();

            _languageDropdown = GameObject.Find("Canvas/Dropdown_Language").GetComponent<Dropdown>();
            _microphoneDevicesDropdown = GameObject.Find("Canvas/Dropdown_MicrophoneDevices").GetComponent<Dropdown>();

            _contextPhrasesInputField = GameObject.Find("Canvas/InputField_SpeechContext").GetComponent<InputField>();
            _operationIdInputField = GameObject.Find("Canvas/InputField_Operation").GetComponent<InputField>();

            _voiceLevelImage = GameObject.Find("Canvas/Panel_VoiceLevel/Image_Level").GetComponent<Image>();
		...
		private void StartRecordButtonOnClickHandler()
		{
			...
			_voiceDetectionToggle.isOn = true;
		...
		private void InsertRecognitionResponseInfo(RecognitionResponse recognitionResponse)
		{
			...
			string resultSpeech = recognitionResponse.results[0].alternatives[0].transcript;
			ActionTriggered(resultSpeech);
			...
			{				
				...
				{					
					...
					//FsmString getString = goFsm.FsmVariables.GetFsmString("getWord");
					//getString.Value = item.word;
					//SendEvent("Get Words");
				}				
				_resultText.text += "\n" + times;				
			}
		...
		private void setApi(string deviceName)
        {						
			if (deviceName != null)
            {
				allDeviceName = new string[2] { "DESKTOP-JMELKQ2", "DESKTOP-9VAG4H8" };
				for (int i = 0; i < allDeviceName.Length; i++)
                {
					if (allDeviceName[i] == deviceName)
						GCSpeechRecognition.Instance.apiKey = "AIzaSyA9FxKUOawATwXJFPau_hfxTsDl4HFH9jg";
				}
            }
        }		
		...
		public void ActionTriggered(string setWord)
		{
			Debug.Log(setWord);
			switch (setWord)
			{
				case "앞으로가.":
					GameEvents.current.Forword();
					break;
				case "뒤로가.":
					GameEvents.current.Back();
					break;
				case "왼쪽으로.":
					GameEvents.current.Left();
					break;
				case "오른쪽으로.":
					GameEvents.current.Right();
					break;
			}
		}
	}
