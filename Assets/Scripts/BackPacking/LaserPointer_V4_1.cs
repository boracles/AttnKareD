...
namespace BNG
{
    ...
    /// </summary>
    public class LaserPointer_V4_1 : MonoBehaviour
    {
        ...
        */
        ...
        private void Awake()
        {
          //  GoFsm.gameObject.GetComponent<PlayMakerFSM>();
            ...
            // Line Renderer is positioned using world s
            if (line1 != null)
            {
...
*/
        ...
        void CaseCheck(GameObject hitobject,bool set)
        {
            if (bPencil)
            {
                if (hitobject.name == "PencilCase_Cap")
                {
                    if (!set)
                    {
                        

                        Cap_arrow.SetActive(false);
                    }
                    if (set)
                    {
                       
                        Cap_arrow.SetActive(true);
                    }
